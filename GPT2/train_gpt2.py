...
class CausalSelfAttention(nn.Module):
        ...
        def __init__(self, config):
            ...
            # not really a 'bias' more of a mask, bit following the OpenAI/HF notation
        ...
        def forward(self, x):
            ...
            # nh is "number of heads", hs is "head size" and C (number of channels) is the embedding dimension = nh * hs
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            ln_f = nn.LayerNorm(config.n_embd)       # This layer helps to stabilize the training process,    # This layer which applies before the Linear layer(lm_head) does not exist in the 'Attention is All You Need' paper but it is used in the GPT-2 paper
        ))
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)                           # Language Model Head (Linear Layer)
...
class DataLoaderLite:
    def __init__(self, B, T):
        ...
        # at init load tokens from disk and store them in RAM
